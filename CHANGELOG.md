...
報酬詳細モーダル
### `/timeline/` Special Voyage ラベル表示修正

#### 変更

- `/timeline/` に表示される Special Voyage 系イベントのラベルを整理。
- `special_voyage_find` などの内部 event_type を、そのまま表示しないようにした。
- 画面上では `Special Voyage` と表示するように変更。

対象イベント：

```txt
special_voyage_find
special_voyage_started
special_voyage_completed
special_voyage_expired
voyage_started
voyage_ended
修正前
special_voyage_find
...
Special Voyage
確認
/timeline/ 上で Special Voyage の漂着物発見イベントが表示されることを確認。
イベント画像が表示されることを確認。
ラベルが special_voyage_find ではなく Special Voyage になっていることを確認。
意図

内部実装名をそのまま見せると、港の記録ではなくシステムログのように見えるため。
Special Voyage に関する出来事は、画面上ではひとつの体験として Special Voyage に統一する。


---

# 2. CHECKLIST.md に追記する場所

`CHECKLIST.md` のこの場所に入れてください。
...
この 「種別表示」セクションの中 に、下を追加してください。

- [ ] `special_voyage_find` が画面上で `Special Voyage` と表示される
- [ ] `special_voyage_started` が画面上で `Special Voyage` と表示される
- [ ] `special_voyage_completed` が画面上で `Special Voyage` と表示される
- [ ] `special_voyage_expired` が画面上で `Special Voyage` と表示される
- [ ] Special Voyage 系イベントで内部 event_type がそのまま表示されていない
- [ ] Special Voyage の漂着物発見イベントに画像が表示される

もし今の CHECKLIST.md の「種別表示」がこうなら：
...
Harbor Cat 自動クローズ
追加
/log/ の Harbor Cat 巻物に自動クローズ処理を追加。
Harbor Cat の返答表示完了後、約7秒で自動的に閉じるようにした。
Harbor Find / Drift Ticket 通知が追加された場合は、最後の通知表示後から約7秒で閉じるようにした。
手動で閉じた場合は、自動クローズタイマーを解除するようにした。
意図

Harbor Cat の返答が「終わったのか、まだ続いているのか」が分かりにくい状態を解消するため。
返答の余韻を少し残しつつ、ユーザーが次の操作へ自然に戻れるようにするため。

/vessels/ 霧の向こう画像対応
変更
次の船・未開放船の画像枠で image pending を表示しないように変更。
実画像を見せない船については、共通の霧画像を表示するようにした。
霧画像として以下を追加。
/images/vessels/vessel-fog.png
表示ルール
所有済み船
  → 実画像を表示
...
未開放船
  ...
  → 実画像・実名・詳細は見せない
修正
次の船カードで画像パスに空文字が渡され、image pending になる問題を修正。
未開放船コレクションで画像パスに空文字が渡され、image pending になる問題を修正。
未開放船の詳細モーダルでも、霧画像を表示できるように調整。
意図

次の船や未開放船の正体は見せないまま、UI上の未完成感を減らすため。
「まだ見えないもの」を、単なる欠落ではなく「霧の向こう」として表現するため。
...
Vessels
所有済み船では実画像が表示されることを確認。
現在の船では実画像が表示されることを確認。
次の船では vessel-fog.png が表示されることを確認。
未開放船では vessel-fog.png が表示されることを確認。
次の船・未開放船で image pending が出ないことを確認。
今後の候補
全内部ページのヘッダー実機確認
