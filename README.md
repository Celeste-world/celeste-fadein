...
現在、各チケット種別の保持上限は1枚です。  
そのため、ユーザー向け表示では以下の表現に統一します。

```txt
同じ航海券は1枚まで

表記ゆれを避けるため、以下のような表現は使い分けません。
...
/tickets/ は、チケット種別ごとの意味を説明するページです。
...
各チケットには、機能説明だけでなく「航海の入口」としての短い文言を表示します。

チケットは、報酬を得るためだけのものではありません。
いつもの記録より少し深く、自分の言葉や海の気配に触れるための入口です。

/account/ での表示

/account/ は、現在のアカウントと、手元に残っている有効な航海券を確認するページです。

各チケットの保持上限が1枚であるため、/account/ では種類ごとの枚数サマリーを表示しません。
...
月10回まで Voyage Log を残せる
Harbor Cat
Harbor Find
Drift Ticket が届く可能性
Items / Vessels / Map / Timeline / Archive の基本機能
...
ユーザー向けには、単なる制限解除ではなく、
港を支えるための支援として説明します。

文言例：

この港を静かに続けるための支援です。
お礼として、今月残せる航海記録が30回増えます。
航海燃料

航海燃料は、たくさん記録を残したい月のための追加燃料です。
...
無料 10回 + 港の維持灯 30回 + 航海燃料 50回
= 月90回まで Voyage Log

文言例：

航海を多く残したい月のための燃料です。
記録の回数が増えても、港の静けさは変わりません。
随時課金チケット

随時課金チケットは、必要な夜だけ深い航海に入るためのものです。
...
その夜だけ、いつもの Voyage Log より深い航海へ入る
...
Drift Ticket は無料で流れ着く短い航海券として扱い、随時課金の対象にはしません。

チケット保持上限

各チケットは、原則として最大1枚まで保持できます。

目的：

チケットを在庫化しすぎない
航海券を「必要な夜に使う入口」として扱う
消費アイテム感を強めすぎない
...
Drift Ticket は無料で港へ流れ着く短い航海券です。
...
単品購入した Special Voyage Ticket / Deep Sea Ticket は、購入日から15日間有効です。

有効期限：購入日から15日

文言例：

購入した航海券は、購入日から15日間有効です。
必要な夜に、桟橋から航海へ出ることができます。
潮の便り
...
潮の便りは、Voyage Log の記録回数を増やしません。
記録回数を増やすものではなく、毎月の航海体験を届けるプランとして扱います。

含まれるもの：

毎月、航海券が1枚届く
Special Voyage Ticket 10分以上を保証
低確率で Special Voyage Ticket 20分 / Deep Sea Ticket が届く
月の航海便
今月の潮
潮の便りのチケット抽選比率

潮の便りでは、Drift Ticket は配布対象に含めません。

Drift Ticket は無料配布の短い航海券であり、
潮の便りでは有料相当の航海券が届くようにします。
...
初回便りについては、月末より契約日から15日後の方が遅い場合、
15日後まで受け取れるようにします。

文言例：

潮の便りは、毎月1日に港へ届きます。
契約した月は、最初の便りがすぐに届きます。

届いた便りは、その月の終わりまで受け取れます。
初回の便りだけは、契約日から少なくとも15日間残ります。
潮の便りとチケット保持上限

チケット本体は最大1枚まで保持できます。
そのため、潮の便りは直接 voyage_tickets に付与せず、まず「受け取り待ち」として届きます。
...
同種チケットをすでに持っている場合の文言例：

手元に同じ航海券があります。
その航海券を使うと、この便りを受け取れるようになります。

受け取れる場合の文言例：

今月の潮の便りが届いています。
Special Voyage Ticket 10分 を受け取れます。

受け取り後：

今月の便りは、すでに航海券として受け取りました。
月の航海便

潮の便りには、月ごとの小さな振り返りを含めます。
...
これは /voyage-archive/ の Monthly Afterglow / Monthly Tags と連動させます。

初期実装では、メール自動送付ではなく、
/voyage-archive/ 内に「月の航海便」として表示する形でもよいです。

将来的には、希望するユーザーへメールで送ることも検討します。

今月の潮

潮の便り契約者には、月初に「今月の潮」を表示します。

例：

今月の潮は、灯台の方を静かに向いています。
急がず、見える小さな光を残す月になりそうです。

これは、毎月の体験に小さな変化を与えるためのものです。
...
Harbor Cat は港側の存在です。  
通常の Voyage Log、漂着物の気配、チケット取得、帰港時の出迎えなどに関わります。

Special Voyage 中の主な返答者は Harbor Cat ではなく、航海中の海です。

### 応答の基本構造

Special Voyage 中の応答は、必ず以下の構造を持たせます。
...
Special Voyage 中の海の応答は、航海記録送信後に中央モーダルで表示します。

目的：

送信直後に手応えを返す
入力後に画面下まで見に行かなくてもよくする
航海中の没入感を保つ

同じ返答は、航海中画面の Special Voyage 返答欄にも記録として残します。
...
Special Voyage 中に漂着物が見つかった場合、海の応答内でも必ず知らせます。

表示方針：

報酬獲得のように煽らない
「波の間から届いた」「この航海の中で見つかった」と表現する
アイテム名は表示してよい
rarity や area_hint などの内部情報は出さない

例：

そのあと、波の間から「月明かりの貝殻」が届きました。
これは、この航海の中で見つかった漂着物です。
既存roll再利用時

Special Voyage の漂着物抽選は、1つの voyage_session につき1回だけ行います。

同じ航海中に複数回記録を書いた場合、既存の voyage_session_find_rolls を再利用します。

この場合、新しく漂着物が届いたようには見せません。

表現例：

この航海で届いたものは、すでに船に収められています。
新しい漂着ではありませんが、その記録はまだ航海の中に残っています。
禁止事項

Special Voyage 中の海の応答では、以下を避けます。
...
special-voyage-reply Edge Function 側で、海の応答を生成します。

フロント側では、Edge Function が返した reply を中央モーダルに表示します。
Edge Function が失敗した場合は、フォールバック文を返し、返答なしにならないようにします。

Drift Ticket の場合は、AI生成が弱い場合でも、入力文を含んだフォールバックで最大3行の応答を返します。

この機能は、有料化を見据えた Special Voyage 体験の中核です。
単に「AIと話す」機能ではなく、ユーザーが置いた言葉に対して、海が確かに応答したと感じられることを目的とします。
